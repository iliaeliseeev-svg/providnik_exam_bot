...
def send_question(chat_id):
    ...
    if q["type"] == "single":
        # ✅ ИСПРАВЛЕНО: убрана лишняя запятая после закрывающей скобки
        buttons = [
    ...
    if q.get("img"):
        try:
            img_value = q["img"]
            # Если это локальный файл — открываем его, если ссылка — отправляем как есть
            if isinstance(img_value, str) and img_value.startswith(("http://", "https://")):
                bot.send_photo(chat_id, img_value, caption=header[:1000], reply_markup=markup)
            else:
                with open(img_value, "rb") as photo:
                    bot.send_photo(chat_id, photo, caption=header[:1000], reply_markup=markup)
            return
        except Exception as e:
            print(f"Ошибка отправки фото: {e}")  # если картинка не загрузилась — отправляем текст
